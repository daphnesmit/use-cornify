...
interface Cornify {
  options: CornifyOptions;
  addKeyboardEvent: (e: KeyboardEvent) => void;
  start: (options?: CornifyOptions) => void;
  add: () => void;
  updateCount: () => void;
...
const cornify: Cornify = {
  ...
  add() {
    ...
    if (cornify.options && (cornify.options.y || cornify.options.younicorns)) {
      url += `&y=${
        cornify.options.y ? cornify.options.y : cornify.options.younicorns
      }`;
    ...
    if (cornifyCount > 5) {
      ...
      if (cornify.options.addMagicalWords) cornify.replace();
    }
  ...
  updateCount() {
    ...
    if (p == null) {
      ...
      p.style.fontFamily =
        "'Comic Sans MS', 'Comic Sans', 'Marker Felt', serif"; // Only the best!
